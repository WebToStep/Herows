...
export const renderHerowDetail = hash => {
    getData()
        .then(data => {
            const filtered = data.filter(i => i.name.replace(/ +/g, '') === hash);
            filtered.forEach(i => {
                console.log('i: ', i);
                let filmList;
                if (i.movies) {
                     filmList = i.movies.map(film => `
                     ...
                     filmList = `
                        ...
                        <div class="card-body p-3">
                        <h5 class="card-title text-gray-900 mb-1">no films detected <i class="fas fa-sad-tear"></i></h5>
                            </div>
                            </div>
                        </div>`;
                }
                        ...
                        ${i.photo ?
                            ...
                            <div class="row">
                            ${i.movies ? filmList.join('') : filmList}
                            </div>
                            </div>
                            </div>`;
